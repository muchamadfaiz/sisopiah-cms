...
export const MYPROJECTS = () => {
  ...
  const getProfile = async () => {
    try {
      ...
      setKabupaten(response.data.user.regency)
    } catch (error) {
      //   setLoading(false);
...
useEffect(() => {
  const getProfileAndSetFilter = async () => {
    try {
      const profile = await dispatch(getUserProfile()).unwrap();
      const userKabupaten = profile.data.user.regency;
      const userRole = profile.data.user.role_id;

      setKabupaten(userKabupaten);
      setRole(userRole);

      setFilters((prev) => ({ ...prev, regency: userKabupaten }));
    } catch (err) {
      message.error("Gagal mengambil profil");
    }
  };

  getProfileAndSetFilter();
}, []); 
